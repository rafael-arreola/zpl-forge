...
impl ZplForgeBackend for PngBackend {
    ...
    fn draw_text(
        ...
        // Rotated text: render on a temporary surface, rotate it, and blit
        // non-white pixels so the white background stays transparent.
        let text_w = self.get_text_width(text, font, height, width).max(1);
        let font_h = (scale_y as u32).max(1);
        let mut tmp: RgbImage = ImageBuffer::from_pixel(text_w, font_h, Rgb([255, 255, 255]));
        draw_text_mut(&mut tmp, text_color, 0, 0, scale, &font_data, text);
